  ...
  const renderTutorialContent = () => {
    switch (trapID) {
      case 0:
        ...
        break;
      case 1:
        setTutorialText("Square brackets [ ] allow you to match a character from a set of characters you specify. \n Example: \n [aeiou] matches any one of the lowercase vowels ('a,' 'e,' 'i,' 'o,' 'u') \n [0-9] matches any digit from 0 to 9."
        );
        break;
      case 2:
        ...
        break;
      case 3:
        setTutorialText("When the caret symbol (^) is used inside a character class (square brackets [ ]), it negates the character class, making it match any character that is not in the specified set. \n Example: \n [^0-9] matches any character that is not a digit. \n [^a-z] matches any character that is not a lowercase letter. \n [^A-D] matches any character that is not A to D inclusive.")
      ...
      case 4:
        ...
        break;
      case 5:
        setTutorialText(" [^2-5] is a character class that matches any single character that is not in the range from '2' to '5'. It essentially excludes characters '2,' '3,' '4,' and '5.' \n Example: \n [0-9] matches any digit from '0' to '9,' \n  [^0-9] matches any character that is not a digit.")
        break;
      case 6:
        setTutorialText("[^] can also be used to filter out or exclude specific characters from your text data. For example, you might want to find all characters that are not digits between '2' and '5' in a string. [^2-5] would match every other character in the string that wasnt 2 - 5 inclusive.")
        break;
      case 7:
        setTutorialText("The {} quantifier allows you to specify exactly how many times a character or group of characters should be repeated in a regular expression. Other Quantifiers: Besides {3} to specify an exact count, you can use other quantifiers like: (*) to match 0 or more occurrences (Example: az*up matches 'aup,' 'azup,' and 'azzzup'). \n  (+) to match 1 or more occurrences (Example:  az+up matches 'azup' and 'azzzup' but not 'aup'). \n (?) to match 0 or 1 occurrence (Example: colou?r matches 'color' and 'colour').")
        break;
      case 8:
        setTutorialText("The + quantifier in a regular expression allows you to match one or more occurrences of the preceding character or group. In the pattern 'c+' matches one or more 'c' characters in sequence. \n Example: \n  'c+' matches 'c' (one 'c'), \n  'cc' matches (two 'c's), \n  'ccc' matches (three 'c's) \n It requires at least one 'c' to match.  'ca+b' matches 'cab,' 'caab,' 'caaab,' and so on because it looks for one or more 'a' characters followed by 'b'. )' ('The + quantifier in a regular expression allows you to match one or more occurrences of the preceding character or group. In the pattern 'c+' it matches one or more 'c' characters in sequence. Examples: 'c+' matches 'c' (one 'c'), 'cc' (two 'c's), 'ccc' (three 'c's), and so on. It requires at least one 'c' to match. 'ca+b' matches 'cab,' 'caab,' 'caaab,' and so on because it looks for one or more 'a' characters followed by 'b'. Quantifier Comparison: * matches 0 or more occurrences (e.g., c* matches '', 'c', 'cc', ...). + matches 1 or more occurrences (e.g., c+ matches 'c', 'cc', 'ccc', ...). ? matches 0 or 1 occurrence (e.g., c? matches '', 'c').")
        break;
      case 9:
        setTutorialText("The caret ^ and dollar sign $ are anchors in regular expressions that specify the position within a line of text. (^) Caret Anchor matches the start of a line. When you use ^ at the beginning of a regular expression, it signifies that the pattern must begin at the very start of a line. ($) Dollar Sign Anchor matches the end of a line. When you use $ at the end of a regular expression, it signifies that the pattern must end at the very end of a line. \n Example: \n (^Hello World) matches 'Hello'. \n  '\?$' this matches lines that end with a question mark ?. The dollar sign $ specifies that the question mark must appear at the very end of the line. \n '^\d+$'  matches lines that contain only digits 0-9. The caret ^ and dollar sign $ together ensure that the entire line consists of digits. (\d matches any digit 0 -9)")
        break;
    }
  };
